// VraagConnect.jsx — Connect-vraag.
// LINKS: niet-selecteerbare referentie (tekst OF afbeelding) met een subtiel
// pijltje (notch) dat naar rechts wijst. Geen omranding bij afbeelding.
// RECHTS: sorteerbare kaartjes (lichtgrijze fill #F9FAFB, geen rand) met
// ↑/↓ icon-buttons. Rij i links hoort bij rij i rechts in de juiste volgorde.
        ...
        {paren.map((p, rowIdx) => {
          ...
          return (
            <React.Fragment key={rowIdx}>
              {/* LINKS — niet-selecteerbaar */}
              {metAfbeelding ? (
                <div style={{
                  position: "relative",
                  height: rowHeight,
                  background: p.afbeelding
                    ? `url(${p.afbeelding}) center / cover no-repeat`
                  ...
                  {!p.afbeelding && <span>Afbeelding vervangen</span>}
                  <ConnectNotch />
                </div>
              ) : (
                <div style={{
                  ...
                  <span style={{ flex: 1 }}>{p.links}</span>
                  <ConnectNotch />
                </div>
              )}

              {/* RECHTS — sorteerbaar, lichtgrijs, geen rand */}
              <div style={{
                minHeight: rowHeight,
                borderRadius: "var(--wt-radius)",
                background: "#F9FAFB",
                ...
                <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                  <ArrowIconBtn dir="up"   disabled={rowIdx === 0}            onClick={() => move(rowIdx, -1)} />
                  <ArrowIconBtn dir="down" disabled={rowIdx === paren.length - 1} onClick={() => move(rowIdx, +1)} />
                </div>
              </div>
            </React.Fragment>
          );
...
// Klein subtiel driehoekje dat rechts uit de linker-kaart "prikt" — zoals in Figma.
function ConnectNotch() {
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <path d="M1 1l10 9-10 9" stroke="currentColor" strokeWidth="1" fill="#FFFFFF" />
    </svg>
  );
}
