// app-main.jsx — App shell: renders all blocks + wires the Tweaks panel.
// Depends on globals:
//   TekstBouwblok, VraagTekst, VraagAfbeeldingen, VraagPoll, VraagStelling,
//   VraagVolgorde, VraagConnect
//   Seg, Toggle, Field, KIND_LABEL, KIND_TWEAKS, MEDIA_SRC, TWEAK_DEFAULTS
...
function renderBlock(b) {
  switch (b.kind) {
    ...
    case "vraag-tekst":
      return (
        <VraagTekst
          ...
          mediaSrc={b.mediaType ? MEDIA_SRC[b.mediaType] : undefined}
          toonPlayknop={b.toonPlayknop !== false}
        />
      );
    case "vraag-afb":
    ...
    default:
      return <div style={{ padding: 48, color: "#999" }}>Onbekend bouwblok: {b.kind}</div>;
  }
}
...
function TweaksPanel({ blocks, activeId, onSelect, onPatch, onAddBlock, onRemoveBlock, settings, onPatchSettings }) {
  ...
  const KindTweaks = KIND_TWEAKS[active.kind] || (() => <div style={{ color: "var(--neutral-500)", fontSize: 12 }}>Geen tweaks voor dit blok.</div>);
  return (
    <>
      ...
      <div className="tweaks-sub">Algemene instellingen</div>
      <div className="tweaks-body" style={{ borderBottom: "1px solid var(--neutral-100)" }}>
        <Field label="Radius van knoppen & opties">
          <Seg
      ...
      <div className="tweaks-sub">Kies een bouwblok</div>
      <div className="tweaks-blocklist">
        {blocks.map((b) => (
          <button
            key={b.id}
          ...
          >
            <span className="dot" />
            <span style={{ flex: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{b.naam}</span>
            <span style={{ color: "var(--neutral-500)", fontSize: 11 }}>
      ...
      <div className="tweaks-sub" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span>Blok toevoegen</span>
      </div>
      <div style={{ display: "flex", flexDirection: "column", borderBottom: "1px solid var(--neutral-100)" }}>
        {KIND_GROUPS.map((group) => {
          const entries = group.kinds
            .filter((k) => KIND_LABEL[k])
            .filter((k) => !window.ALLOWED_KINDS || window.ALLOWED_KINDS.includes(k))
            .map((k) => [k, KIND_LABEL[k]]);
          if (entries.length === 0) return null;
          return (
            <div key={group.label} style={{ padding: "6px 10px 10px" }}>
              <div style={{
      ...
      <div className="tweaks-sub">Variabelen — {active.naam}</div>
      <div className="tweaks-body">
        <Field label="Naam van het blok">
          <input
            className="tw-text"
            value={active.naam}
        ...
        <KindTweaks b={active} patch={(p) => onPatch(active.id, p)} />
        {blocks.length > 1 && (
          <button
...
function templateFor(kind) {
  ...
  switch (kind) {
    case "tekst":
      return {
        ...base, variant: "een-kolom", achtergrond: "standaard", onderSpacing: true,
        mediaType: "afbeelding", mediaDimensions: "boxed", toonPlayknop: true,
        titel: "Titel", subtitel: "Subtitel",
        body: "Lorem ipsum dolor sit amet, consectetur adipiscing elit.",
        body2: "", toonTitel: true, toonSubtitel: true,
      };
    case "quote":
      return {
        ...base, achtergrond: "licht", onderSpacing: true,
    ...
    case "external-link":
      return {
        ...base, achtergrond: "standaard", onderSpacing: true,
        type: "link",
        titel: "Verder lezen",
    ...
    case "stepper":
      return {
        ...base, achtergrond: "donker", onderSpacing: true,
        layout: "twee-kolommen",
        stappen: [
          { titel: "Stap één",   subtitel: "Oriëntatie",  body: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis et augue sed nunc ullamcorper venenam." },
    ...
    case "vraag-tekst":
      return {
        ...
        multi: false, toonFeedback: false,
        layout: "alleen-tekst",
        mediaType: "afbeelding", toonPlayknop: true,
        opties: [
    ...
    case "vraag-connect":
      return {
        ...
        metAfbeelding: false, toonFeedback: false,
        paren: [
...
function App() {
  ...
  // Apply settings.radius as --wt-radius on <html>
  React.useEffect(() => {
  ...
  // Keep a ref for removeBlock to avoid dep-cycle
  ...
  return (
    <>
      {state.blocks.map((b) => (
        ...
        >
          <div className="block-label">{b.naam} · {KIND_LABEL[b.kind] || b.kind}</div>
          {renderBlock(b)}
        </div>
      ))}
